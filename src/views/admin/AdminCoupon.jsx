    ...
    <div className="min-h-screen bg-[#FAF9F6] px-3 py-3 md:px-6 md:py-12 font-sans text-[#111111]">
      ...
      <div className="max-w-7xl mx-auto mb-6 md:mb-20">
        <div className="flex flex-row md:items-end justify-between gap-3 md:gap-8 border-b border-[#D1C7B7] pb-3 md:pb-10 relative">
          ...
          <button
            className="group relative px-3 md:px-10 py-2 md:py-4 overflow-hidden transition-all duration-700 hover:shadow-2xl shrink-0 self-end"
          ...
          >
            ...
            <span className="relative text-[0.75rem] uppercase tracking-[0.2em] md:tracking-[0.4em] font-bold text-[#111111] group-hover:text-[#FAF9F6] transition-colors duration-700 whitespace-nowrap">
              簽署新優惠 · ISSUE
            </span>
          </button>
        </div>
      ...
      {/* Summary Stats — 手機/平板：合併成一張，桌機：兩張 */}
      <div className="max-w-7xl mx-auto lg:hidden mb-6 md:mb-12 p-3 md:p-6 bg-white border border-[#D1C7B7]/30">
        <div className="grid grid-cols-2 divide-x divide-[#D1C7B7]/30">
          <div className="px-3">
            <span className="block text-[12px] uppercase tracking-[0.2em] font-bold opacity-30 mb-2">
              現行優惠
            </span>
            <span className="text-3xl font-serif text-[#111111]">
              {coupons.length}
            </span>
            <span className="text-[12px] ml-1 opacity-30">Active</span>
          </div>
          <div className="px-3">
            <span className="block text-[12px] uppercase tracking-[0.2em] font-bold opacity-30 mb-2">
              折扣效應
            </span>
            <span className="text-3xl font-serif text-[#3A4D39]">85%</span>
            <span className="text-[12px] ml-1 opacity-30">Average</span>
          </div>
        </div>
      </div>
      <div className="max-w-7xl mx-auto hidden lg:grid lg:grid-cols-4 gap-3 md:gap-12 mb-8 md:mb-24">
        <div className="p-3 md:p-8 border-l border-[#D1C7B7]/30 hover:border-[#111111] transition-colors duration-500">
          <span className="text-[0.75rem] uppercase tracking-wider md:tracking-widest font-bold opacity-30 block mb-2">
            現行優惠總數
          </span>
          <span className="text-4xl font-serif text-[#111111]">
            {coupons.length}{' '}
            <small className="text-xs font-sans opacity-20 font-normal">
              Active
            </small>
          </span>
        </div>
        <div className="p-3 md:p-8 border-l border-[#D1C7B7]/30 hover:border-[#3A4D39] transition-colors duration-500">
          <span className="text-[0.75rem] uppercase tracking-wider md:tracking-widest font-bold opacity-30 block mb-2">
            折扣效應評核
          </span>
          <span className="text-4xl font-serif text-[#3A4D39]">
            85%{' '}
            <small className="text-xs font-sans opacity-20 font-normal">
              Average
            </small>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-[#D1C7B7]/30 text-[10px] md:text-[0.75rem] uppercase tracking-[0.1em] md:tracking-[0.3em] font-bold text-[#111111]/40">
                <th className="px-2 py-3 md:px-4 md:py-8">活動標題 / DESC</th>
                <th className="px-2 py-3 md:px-4 md:py-8 hidden md:table-cell">
                  優惠代碼 / CODE
                </th>
                <th className="px-2 py-3 md:px-4 md:py-8">折扣權重 / VALUE</th>
                <th className="px-2 py-3 md:px-4 md:py-8 hidden md:table-cell">
                  有效期限 / EXPIRY
                </th>
                ...
                coupons.map((coupon) => (
                  ...
                  >
                    <td className="px-2 py-3 md:px-4 md:py-10">
                      ...
                      <div className="text-[0.75rem] opacity-20 font-mono tracking-tighter mt-1">
                        REF: {coupon.id.slice(-10)}
                      </div>
                    </td>
                    <td className="px-2 py-3 md:px-4 md:py-10 hidden md:table-cell">
                      <code className="text-[0.85rem] font-mono font-bold text-[#111111] tracking-wider md:tracking-widest bg-[#FAF9F6] border border-[#D1C7B7]/40 px-3 py-1 group-hover:border-[#111111] transition-colors">
                        {coupon.code}
                      </code>
                    </td>
                    <td className="px-2 py-3 md:px-4 md:py-10">
                      ...
                      <span className="hidden md:inline text-[0.75rem] uppercase tracking-wider md:tracking-widest font-black ml-2 opacity-20">
                        Percent Off
                      </span>
                      <span className="md:hidden text-[10px] opacity-20 ml-1">
                        %
                      </span>
                    </td>
                    <td className="px-2 py-3 md:px-4 md:py-10 hidden md:table-cell">
                    ...
                    <td className="px-2 py-3 md:px-4 md:py-10 text-center">
                      <div className="flex flex-col items-center gap-1">
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${coupon.is_enabled ? 'bg-[#3A4D39]' : 'bg-[#111111]/10'}`}
                        ></span>
                        <span
                          className={`text-[0.75rem] uppercase tracking-[0.2em] font-bold ${coupon.is_enabled ? 'text-[#3A4D39]' : 'opacity-10'}`}
                        >
                    ...
                    <td className="px-2 py-3 md:px-4 md:py-10 text-right">
                      <div className="inline-flex gap-3 md:gap-8">
                        <button
                          className="text-[0.75rem] uppercase tracking-[0.1em] md:tracking-[0.3em] font-bold text-[#111111]/40 hover:text-[#111111] transition-colors"
                          onClick={() => openModal('edit', coupon)}
                        >
                          修訂
                        </button>
                        <button
                          className="text-[0.75rem] uppercase tracking-[0.1em] md:tracking-[0.3em] font-bold text-[#984443]/40 hover:text-[#984443] transition-colors"
            ...
            <div className="flex-grow p-3 md:p-12 overflow-y-auto custom-scrollbar bg-white">
              <div className="max-w-md mx-auto space-y-12">
                ...
                <section className="space-y-4">
                  <label className="text-[0.75rem] uppercase tracking-[0.15em] md:tracking-[0.4em] font-black text-[#111111]/30 block px-1">
                    活動名錄 / NOMENCLATURE
                  </label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-12">
                  ...
                  <section className="space-y-4">
                    ...
                    <div className="relative group">
                      ...
                      />
                      <span className="absolute right-0 bottom-2 text-xs opacity-20">
                        % OFF
                      </span>
                    </div>
                  </section>
                ...
                <section className="pt-8 border-t border-[#D1C7B7]/10">
                  <label className="flex items-center gap-6 cursor-pointer group">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[0.75rem] uppercase tracking-[0.2em] font-black text-[#111111] opacity-40 group-hover:opacity-100 transition-opacity">
                        生效宣告 · PUBLISH
                      </span>
